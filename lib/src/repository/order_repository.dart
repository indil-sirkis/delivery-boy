...
Future<Stream<Order>> getOrders() async {
  ...
  _queryParams['search'] = 'active:1;driver.id:${_user.id};order_status_id:$orderStatusId;delivery_address_id:null';
  _queryParams['searchFields'] = 'active:=;driver.id:=;order_status_id:<>;delivery_address_id:<>';
...
Future<Stream<Order>> getOrdersHistory() async {
  ...
  _queryParams['api_token'] = _user.apiToken;
  _queryParams['with'] = 'driver;productOrders;productOrders.product;productOrders.options;orderStatus;deliveryAddress;payment';
  _queryParams['search'] = 'active:=<0,1>;driver.id:${_user.id};order_status_id:$orderStatusId;delivery_address_id:null';
  _queryParams['searchFields'] = 'active:=;driver.id:=;order_status_id:=;delivery_address_id:<>';
...
Future<Order> deliveredOrder(Order order) async {
  ...
  final client = new http.Client();
  print("DELIVER::${url}");
  print("DELIVER_PARM::${json.encode(order.deliveredMap())}");
